...
const AchievementsAdmin: React.FC = () => {
  ...
  const addItem = () => setItems(prev => [...prev, { title: '', date: '', description: '', icon: 'trophy' }]);
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={addItem}
            className="group relative px-6 py-3 admin-button-secondary flex items-center space-x-2"
          >
      ...
      <div className="space-y-6 animate-slide-up">
        {items.map((item, i) => (
          <div
            key={i}
            className="group relative glass-morphism rounded-2xl shadow-xl overflow-hidden admin-card-hover"
          >
            {/* Card Header */}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                    Competition #{i + 1}
                    {item.title && <span className="text-purple-600 dark:text-purple-400"> - {item.title}</span>}
                  </h3>
                </div>
                <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Competition Icon</label>
                  <div className="relative">
                    <select
                      value={item.icon || 'trophy'}
                      onChange={(e) => updateItem(i, 'icon', e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 appearance-none"
                    >
                      <option value="trophy">🏆 Trophy (Championship/Winner)</option>
                      <option value="medal">🏅 Medal (Achievement/Rank)</option>
                      <option value="award">🎖️ Award (Recognition)</option>
                      <option value="star">⭐ Star (Excellence)</option>
                      <option value="crown">👑 Crown (Champion)</option>
                      <option value="target">🎯 Target (Goal Achievement)</option>
                    </select>
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setSuccess(`Image uploaded successfully for competition ${i + 1}`);
                        ...
                        className="sr-only"
                      />
                      <div className={`flex items-center justify-center px-6 py-4 border-2 border-dashed rounded-xl transition-colors duration-200 ${
                        uploadingIndex === i 
      ...
      {items.length === 0 && (
        <div className="text-center py-12">
          ...
          <button
            onClick={addItem}
            className="px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center space-x-2"
          >
            <Trophy className="w-5 h-5" />
